...
=pod
   ...
   my $sum_id = 
      $sum->add( 'adfasdfasdfasfdadf' );

   my $sum_id = 
      $sum->get( 'adfasdfasdfasfdadf' );

   my $sum = 
      $sum->get( 5 );

   $sum->delete( 'adfasdfasdfasfdadf' );
   $sum->delete( 5 );
...
Argument is id or sum.
Returns bool.

=head2 get()

If argument is sum, returns id. 
If argument is id, returns sum.
